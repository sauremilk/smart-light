...
HUE_LIGHT_IDS = [2, 3, 4, 6]      # Mick Zimmer 1, Mick Zimmer 3, Mick Zimmer 2, Hue lightstrip 1
...
STARTUP_GUARD_DELAY_SECONDS = 4.0  # FPS-Regelung erst nach Initialisierungsphase aktivieren

# === DeepFace ===
DETECTOR_BACKEND = "opencv"         # opencv ist deutlich leichter und haelt FPS stabiler
MIN_CONFIDENCE = 0.55               # Mindest-Confidence fuer Emotion (von 0.45 erhoeht → weniger Rauschen)
SOFT_MIN_CONFIDENCE = 0.30          # Untere Grenze: darunter wird ein Frame als zu unsicher verworfen
LOW_CONFIDENCE_ALPHA_SCALE = 0.35   # Wie stark unsichere Frames (zwischen SOFT_MIN und MIN) EMA beeinflussen
...
UNCERTAINTY_MARGIN_WEIGHT = 0.60    # Hoeher => klare Klassenabstaende werden staerker belohnt
UNCERTAINTY_ENTROPY_WEIGHT = 0.40   # Hoeher => flache Verteilungen werden staerker abgestraft
...
USE_COLOR_CONSTANCY = True          # Gray-World-Korrektur vor CLAHE (bricht Licht→Kamera-Feedback-Loop)
CLAHE_CLIP_LIMIT = 2.0             # CLAHE Contrast-Limit (0=aus). Normalisiert ungleichmaessige Beleuchtung.
...
EMA_MIN_WEIGHT = 0.05              # Emotionen unter 5% EMA-Gewicht werden beim Farbblending ignoriert
FALLBACK_DECAY = 0.08              # Wie schnell EMA-Vektor Richtung neutral driftet wenn kein Gesicht erkannt
...
ABSENCE_LIGHT_OFF_SECONDS = 180     # Sekunden ohne Gesicht im Bild → Licht ausschalten (3 Minuten)
...
USE_VALENCE_AROUSAL = True          # True: Licht ueber Valence/Arousal steuern. False: direktes Emotion-Mapping.
...
AUDIO_DYNAMIC_MIN_FACTOR = 0.20    # Minimaler Anteil vom AUDIO_WEIGHT bei schlechter Qualitaet
...
BREATHING_WINDOW_SECONDS = 30.0    # Rollendes Zeitfenster in Sekunden (mind. 2–3 Atemzyklen)
BREATHING_FRAME_INTERVAL = 6       # Alle N Frames einen Frame schicken (24fps → ~4 Hz Abtastrate)
...
BREATHING_TRANSITION_INFLUENCE = 0.8   # Positives Offset beschleunigt, negatives verlangsamt Transition
...
ADAPTIVE_REGULATION = True            # True: adaptiver Modus aktiv; False: klassischer Spiegel-Modus
...
ADAPTIVE_PROGRESS_TIMEOUT = 30.0      # Sekunden ohne Verbesserung → blend um ESCALATION erhöhen
ADAPTIVE_BLEND_ESCALATION = 0.10      # Schrittweite je Eskalationsstufe
ADAPTIVE_AT_TARGET_THRESHOLD = 0.18   # Distanz zum Ziel < Schwelle → "Stabil", kein Eingriff
