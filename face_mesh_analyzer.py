...
class FaceMeshAnalyzer:
    ...
    def _aus_to_emotions(aus: dict) -> dict:
        ...
        scores["sad"] = min(100.0, (aus["AU1"] * 0.3 + aus["AU4"] * 0.3 + aus["AU15"] * 0.4) * 60.0)

        # Angry: AU4 (Brow Lowerer) + AU9 (Nose Wrinkler) stark, wenig AU1
        scores["angry"] = min(100.0, (aus["AU4"] * 0.5 + aus["AU9"] * 0.3) * 50.0
                               * max(0.2, 1.0 - aus["AU1"]))

        # Fear: AU1 + AU2 (Brauen hoch) + AU4 + AU20 (Lip Stretcher)
        scores["fear"] = min(100.0, (aus["AU1"] * 0.3 + aus["AU2"] * 0.2
                                      + aus["AU4"] * 0.2 + aus["AU20"] * 0.3) * 55.0)

        # Surprise: AU1 + AU2 (Brauen hoch) + AU25 (Lips Part) + AU26 (Jaw Drop)
        scores["surprise"] = min(100.0, (aus["AU1"] * 0.2 + aus["AU2"] * 0.2
                                          + aus["AU25"] * 0.3 + aus["AU26"] * 0.3) * 50.0)
